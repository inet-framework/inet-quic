...
simtime_t OscillatorBasedClock::computeSimTimeFromClockTime(clocktime_t clockTime, bool lowerBound) const
{
    ...
    simtime_t result = oscillator->getComputationOrigin() + (originSimulationTime - oscillator->getComputationOrigin()) + (oscillator->getComputationOrigin() + simulationTimeInterval - originSimulationTime) / (1 + getOscillatorCompensation().get<unit>());
